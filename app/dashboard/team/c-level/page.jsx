...
export default function CLevelTeam() {
  ...
  // Filter for C-level members (level 3)
  const cLevelMembers = teamData?.teamMembers?.filter(member => member.level === 3) || []
  
  // Filter by search term if any
  const filteredMembers = searchTerm 
    ? cLevelMembers.filter(member => 
      ...
      <div className="min-h-screen bg-background">
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center mb-6">
            ...
            <h1 className="text-2xl font-bold">C-Level Team Members</h1>
          </div>
            ...
            >
              <div className="bg-background rounded-lg border border-border/40 shadow-sm p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-muted-foreground">C-Level Members</p>
                    <h3 className="text-2xl font-bold mt-1">{cLevelMembers.length}</h3>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-2xl font-bold mt-1 text-blue-500">
                      {teamData?.commissionPercentages?.C 
                        ? `${(teamData.commissionPercentages.C * 100).toFixed(1)}%` 
                        : "Loading..."}
                    </h3>
                  </div>
                  <div className="h-12 w-12 rounded-full bg-blue-500/10 flex items-center justify-center">
                    <span className="text-sm font-bold text-blue-500">C</span>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <div className="flex gap-2">
              ...
              <Link href="/dashboard/team/c-level">
                <button className="px-4 py-2 rounded-lg bg-primary text-white">C-Level</button>
              </Link>
          ...
          <div className="bg-background rounded-lg border border-border/40 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-border/40">
              <h2 className="text-xl font-semibold">Your C-Level Team Members</h2>
              <p className="text-sm text-muted-foreground mt-1">
                Members in your third level of the network
              </p>
            </div>
            ...
            {loading ? (
              ...
              <div className="p-12 text-center">
                ...
                <h3 className="text-lg font-medium">No C-Level members yet</h3>
