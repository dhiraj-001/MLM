...
export default function DLevelTeam() {
  ...
  // Filter for D-level members (level 4)
  const dLevelMembers = teamData?.teamMembers?.filter(member => member.level === 4) || []
  
  // Filter by search term if any
  const filteredMembers = searchTerm 
    ? dLevelMembers.filter(member => 
      ...
      <div className="min-h-screen bg-background">
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center mb-6">
            ...
            <h1 className="text-2xl font-bold">D-Level Team Members</h1>
          </div>
            ...
            >
              <div className="bg-background rounded-lg border border-border/40 shadow-sm p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-muted-foreground">D-Level Members</p>
                    <h3 className="text-2xl font-bold mt-1">{dLevelMembers.length}</h3>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-2xl font-bold mt-1 text-amber-500">
                      {teamData?.commissionPercentages?.D 
                        ? `${(teamData.commissionPercentages.D * 100).toFixed(1)}%` 
                        : "Loading..."}
                    </h3>
                  </div>
                  <div className="h-12 w-12 rounded-full bg-amber-500/10 flex items-center justify-center">
                    <span className="text-sm font-bold text-amber-500">D</span>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <div className="flex gap-2">
              ...
              <Link href="/dashboard/team/d-level">
                <button className="px-4 py-2 rounded-lg bg-primary text-white">D-Level</button>
              </Link>
            </div>
          </div>
          ...
          <div className="bg-background rounded-lg border border-border/40 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-border/40">
              <h2 className="text-xl font-semibold">Your D-Level Team Members</h2>
              <p className="text-sm text-muted-foreground mt-1">
                Members in your fourth level of the network
              </p>
            </div>
            ...
            {loading ? (
              ...
              <div className="p-12 text-center">
                ...
                <h3 className="text-lg font-medium">No D-Level members yet</h3>
                <p className="text-muted-foreground mt-2 max-w-md mx-auto">
                  You'll see members here as your network expands to the fourth level.
                </p>
              </div>
            ) : (
